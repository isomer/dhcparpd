...
dhcpctl_handle dhcpd_connect( char *hostname, int port,
                 char* username,  unsigned char *key)
{
        dhcpctl_status status;
	dhcpctl_handle conn = NULL;
...
%extend Omapi {
	Omapi( char *hostname, int port,  char *username,
		 char *key) {
		Omapi *o;
	...
	char *ret;
        ...
        if ((status = dhcpctl_new_object(&hp, self->conn, "host")))
                dhcpctl_error("host create failed", status);
	...
	memcpy(ds->value,ether_aton(mac),sizeof(struct ether_addr));
        if ((status = dhcpctl_set_value(hp, ds, "hardware-address")))
                dhcpctl_error("Failed to set hardware-address",status);
	...
	if ((status= dhcpctl_set_int_value(hp, 1,"hardware-type")))
		dhcpctl_error("Failed to set hardware-type",status);

        if ((status = dhcpctl_open_object(hp, self->conn, DHCPCTL_CREATE|DHCPCTL_EXCL)))
                dhcpctl_error("Failed to create",status);
	...
	if ((status = dhcpctl_set_int_value(hp, 1, "hardware-type")))
        ...
        if ((status = dhcpctl_open_object(hp, self->conn, 0)))
                dhcpctl_error("Failed to refresh query (open)",status);

        if ((status = dhcpctl_wait_for_completion(hp, &status2)))
                dhcpctl_error("Completion failed (open)",status);

	if (status2)
		dhcpctl_error("delete (open)",status2);

        if ((status = dhcpctl_object_remove(self->conn, hp)))
	...
	char *ret;
        ...
        if ((status = dhcpctl_new_object(&hp, self->conn, "lease")))
